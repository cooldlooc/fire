...
const COLOR = {
	...
	Gold: '#ffbf36',
	White: '#ffffff',
	// Dim amber — reserved for the text-reveal effect so the letters read softer than a
	// normal burst. Excluded from randomColor() so regular shells won't pick it.
	Amber: '#b37a22'
};
...
function randomColorSimple() {
	...
	// Amber is reserved for the text-reveal effect; resample if randomly picked.
	while (c === COLOR.Amber) c = COLOR_CODES[Math.random() * COLOR_CODES.length | 0];
	return c;
}
...
// Text effect — renders a string to an offscreen canvas, then samples the filled pixels
// into normalized (dx, dy) target positions. The burst code uses those positions as
// destinations for dust left behind by the first explosion.
const TextEffect = {
	ready: false,
	text: 'LZW chill out',
	points: [],     // { dx, dy } offsets from text centroid, in units of font cap height.
	aspect: 1,      // text width / font height — used to size the result on screen.
	...
	build() {
		...
		ctx.fillText(this.text, pad, pad);
		const data = ctx.getImageData(0, 0, w, h).data;

		// Sample on a regular stride to keep particle count manageable while preserving letterforms.
		const stride = 4;
		const pts = [];
		...
		cx /= pts.length;
		cy /= pts.length;
		// Normalize relative to font size so on-screen sizing scales cleanly.
		this.points = pts.map(p => ({ dx: (p.x - cx) / fontSize, dy: (p.y - cy) / fontSize }));
		this.aspect = w / fontSize;
		this.ready = true;
	},
};
...
const textShell = (size=1) => {
	if (!TextEffect.ready) TextEffect.build();
	return {
		shellSize: size,
		text: true,
		color: COLOR.Gold,
		spreadSize: 360 + size * 120,
		starLife: 3200 + size * 260,
	...
	COLOR_CODES_W_INVIS.forEach(color => {
		...
		for (let i=stars.length-1; i>=0; i=i-1) {
			...
			if (star.life <= 0) {
				...
				const age = star.fullLife - star.life;
				if (star.textFall && age < star.fallDelay) {
					star.visible = false;
				}
				else {
					const holdTextShape = star.textFall && age < star.fallStartDelay;
					if (star.textFall) {
						star.visible = true;
					}
					if (!holdTextShape) {
						if (star.textFall && !star.fallStarted) {
							star.fallStarted = true;
							star.speedY = star.fallSpeed;
						}
		...
		stars.forEach(star => {
			if (star.visible) {
				...
				if (!star.noMain) {
					...
					trailsCtx.moveTo(star.x - 0.25, star.y);
					trailsCtx.lineTo(star.x + 0.25, star.y);
				}
			}
		});
		trailsCtx.stroke();
...
class Shell {
	...
	burst(x, y) {
		...
		// Text burst: a real chrysanthemum-like explosion plays first, then the dust it
		// leaves behind drifts into the "LZW chill out" letterforms.
		if (this.text && TextEffect.ready) {
			// Size the rendered text generously, but cap it so it doesn't overflow the stage.
			const maxWidth = stageW * 0.86;
			const targetWidth = Math.min(this.spreadSize * 1.6, maxWidth);
			const fontHeight = targetWidth / TextEffect.aspect;

			// Phase 1: outward debris explosion (visible from t=0). This is the "boom".
			const debrisSpeed = this.spreadSize / 96;
			const debrisCount = Math.max(80, Math.min(260, Math.round(this.starCount * 0.6)));
			createBurst(debrisCount, (angle, speedMult) => {
				const star = Star.add(
					x,
					y,
					this.color,
					angle,
					speedMult * debrisSpeed * (0.7 + Math.random() * 0.5),
					600 + Math.random() * 360
				);
				// Fade quickly so the debris doesn't muddy the text once it appears.
				star.transitionTime = 260;
				star.secondColor = INVISIBLE;
			});

			// Phase 2: dim amber dust appears in the sampled text points, then falls under
			// gravity. The stagger makes the letters feel like lingering debris, not a hard cut.
			const textLife = this.starLife;
			const textColor = COLOR.Amber;
			for (const pt of TextEffect.points) {
				const tx = x + pt.dx * fontHeight;
				const ty = y + pt.dy * fontHeight;
				const life = textLife + Math.random() * textLife * this.starLifeVariation;
				const star = Star.add(
				...
				star.noMain = true;
				star.textFall = true;
				star.fallDelay = 420 + Math.random() * 260;
				star.fallStartDelay = 1250;
				star.fallSpeed = 0.018;
				star.transitionTime = Math.max(1, life - 2600 - Math.random() * 420);
				star.secondColor = INVISIBLE;
			}
...
const Star = {
	...
	add(x, y, color, angle, speed, life, speedOffX, speedOffY) {
		...
		instance.noMain = false;
		instance.textFall = false;
		instance.fallStarted = false;
		instance.fallStartDelay = 0;
